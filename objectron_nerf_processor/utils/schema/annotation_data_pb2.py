...
import sys
# If the current file is being run directly, or if it's imported as a top-level module,
# the relative import might fail. We add the parent directory to sys.path to ensure
# 'a_r_capture_metadata_pb2' is discoverable if it's in the same directory.
# This is a common workaround for protobuf generated files that might be used
# in different import contexts.

try:
    from . import a_r_capture_metadata_pb2 as a__r__capture__metadata__pb2
    from . import object_pb2  as object__pb2
except ImportError:
    # Fallback for environments where direct relative import fails (e.g., if schema is not a package)
    # This might happen if 'annotation_data_pb2.py' is run as __main__ or imported in a non-standard way.
    import os
    import sys
    sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__))))
    import a_r_capture_metadata_pb2 as a__r__capture__metadata__pb2
    import object_pb2  as object__pb2
    sys.path.pop(0) # Remove it after import to keep sys.path clean
